...
@app.get("/stock/{symbol}/technical", response_model=TechnicalIndicators)
async def get_technical_indicators(symbol: str):
    try:
        ...
        rs = gain / loss
        rsi = 100 - (100 / (1 + rs)).fillna(50)  # Default RSI to neutral (50) if insufficient data
        
        # Calculate MACD
        exp1 = hist['Close'].ewm(span=12, adjust=False).mean()
        exp2 = hist['Close'].ewm(span=26, adjust=False).mean()
        macd = (exp1 - exp2).fillna(0)
        signal = macd.ewm(span=9, adjust=False).mean().fillna(0)
        hist_macd = (macd - signal).fillna(0)
        
        # Calculate SMAs
        sma_20 = hist['Close'].rolling(window=20).mean().fillna(hist['Close'].iloc[0])
        sma_50 = hist['Close'].rolling(window=50).mean().fillna(hist['Close'].iloc[0])
...
@app.get("/stock/{symbol}/info", response_model=CompanyInfo)
...
@app.get("/stock/{symbol}/major_holders")
    ...
    try:
        ...
        for index, row in major_holders.iterrows():
            if len(row) < 2:
                continue
            holders_list.append({
                "category": str(row[0]),  # Convert to string
                "percentage": float(row[1]) if isinstance(row[1], (int, float)) else 0  # Ensure numeric value
            })
    ...
    try:
        ...
        for index, row in earnings.iterrows():
            earnings_list.append({
                "date": index.isoformat() if isinstance(index, datetime) else str(index),
                "revenue": float(row['Revenue']) if 'Revenue' in row and pd.notna(row['Revenue']) else 0,
                "earnings": float(row['Earnings']) if 'Earnings' in row and pd.notna(row['Earnings']) else 0,
                "earnings_per_share": float(row['Earnings Per Share']) if 'Earnings Per Share' in row and pd.notna(row['Earnings Per Share']) else 0
            })
...
@app.get("/nifty50", response_model=Dict)
...
async def search_stocks(query: str):
    try:
        ...
        if not info or 'longName' not in info:
            raise HTTPException(status_code=404, detail=f"Stock symbol {query} not found")
